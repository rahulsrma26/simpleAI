...
NeuralNetwork::NeuralNetwork(uint input, LossFunction _cost) : inputNeurons(input), cost(_cost) {}

uint NeuralNetwork::inputSize() const { return inputNeurons; }

size_t NeuralNetwork::numLayers() const { return layers.size() + 1; }

uint NeuralNetwork::outputSize() const { return layers.size() ? layers.back()->size() : -1; }
...
void NeuralNetwork::train(const realMatrix& input, const realMatrix& label, uint batchSize,
                          real learningRate) {
